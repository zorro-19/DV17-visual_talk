...
static const struct db_table app_config_table[] = {
    ...
    {"screen",  4,      1},            // 背景图
    {"reset_state",  1,      0},            // 背景图
    {"bound_state",  1,      0},            // 设备绑定状态, 方式未联网情况下解除绑定
...
OS_SEM init_video_sem;
int os_query_init_video_sem(void)
{
    if (os_sem_query(&init_video_sem)) {
        return 0;
    }
    return os_sem_pend(&init_video_sem, 100);
}

void init_video_task(void *priv)
{
    u32 t = timer_get_ms();
    void *dev = dev_open("video0.0", NULL);
    if (dev) {
        dev_close(dev);
        os_sem_set(&init_video_sem, 8);
        os_sem_post(&init_video_sem);
        os_sem_post(&init_video_sem);
    }
    u32 t1 = timer_get_ms();
    printf("\n >>>>>>>>>>>>init_video_time = %d\n", t1 - t);
...
#ifdef CONFIG_VIDEO0_INIT_ASYNC
    os_sem_create(&init_video_sem, 0);
    task_create(init_video_task, NULL,"init_video_task");
#endif
