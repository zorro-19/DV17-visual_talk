...
#define UART_DEFAULT_BAUD		(50*10000L)//9600
#define UART_UPDATE_BAUD		(50*10000L)//9600//(50*10000L)
...
u32 update_baudrate =(50*10000L);//9600;             //初始波特率
...
extern int spec_uart_upg_send(char *buf, u32 len);
...
static bool uart_send_packet(u8 *buf, u16 length)
{
   ...
   // printf("\n uart_send_packet start \n");
  //  put_buf((u8 *)&protocal_frame,length + SYNC_SIZE);
    spec_uart_upg_send((u8 *)&protocal_frame, length + SYNC_SIZE);
    uart_set_dir(1);

   // printf("\n uart_send_packet end \n");
    return ret;
}
...
static u32 update_data_read_from_file(void *p, u32 addr, u32 len)
{
   ...
   // printf("\n len + sizeof(struct file_info):%d\n",len + sizeof(struct file_info));
...
static void update_process_run(void)
{
    ...
    while (1) {
        if (OS_NO_ERR != os_sem_pend(&__this->rx_sem, 800)) {
            ...
            update_baudrate =	(50*10000L);// 9600;
            uart_update_set_baud(update_baudrate);
            continue;
        }
                ...
                if (__this->update_percent >= 99) {
                 ...
                 if(!up_flag){
                    ...
                    printf("\n enter_upgrade_ui\n ");
                      ...
                      post_msg_doorbell_task("doorbell_event_task", 2, DOORBELL_EVENT_DEVICE_UART_UPGRADE_UI_CMD, 1);
                    #endif // 1
                 }
                printf("send data process:%d,%x\n", __this->update_percent, __this->update_percent);
            }
...
static int uart_upgrade=0;
void uart_update_init()
{


    //thread_fork(THIS_TASK_NAME, 18, 256, 256, 0, update_loader_download_task, NULL);
    thread_fork(THIS_TASK_NAME, 15, 256, 256, &uart_upgrade, update_loader_download_task, NULL);


    extern void spec_uart_init(void *priv);
    spec_uart_init(NULL);
...
void uart_update_exit(void)
{
   ...
   // task_kill(THIS_TASK_NAME);
    thread_kill(&uart_upgrade, KILL_WAIT);
}
