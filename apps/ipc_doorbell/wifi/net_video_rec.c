...
void *get_net_video_rec_hdl(void)
{
    return __this_net->net_video_rec;
}
...
int user_net_video_rec_open(char forward)
{
    ...
    if (qos >= 16 && qos <= 20) {
        ...
        info.width = GET_VIDEO_WIDTH;
        info.height = GET_VIDEO_HEIGHT;
        #endif
...
int video_rec_finish_notify(char *path)
{
    ...
    printf("\n video_rec_finish_notify \n");
    extern int cloud_playback_list_add(char *path);
    cloud_playback_list_add(path);
...
#else
    char name[128];
    fget_name(fd, name, 64);
    extern int cloud_playback_list_local_remove(char *name);
    cloud_playback_list_local_remove(name);
...
static int net_video_rec0_start()
{
...
#else
#ifdef CONFIG_SHARED_REC_HDL_ENABLE
        __this_net->net_video_rec = server_open("video_server", "video0.0");
#else
    ...
    req.rec.audio.volume    = __this_net->net_video0_art_on ? net_video_rec_get_adc_volume() : 0;
...
//    }
    req.rec.cycle_time = 15 * 60;//db_select("cyc");
...
static int net_video_rec1_start()
{
...
#ifdef CONFIG_SHARED_REC_HDL_ENABLE
        __this_net->net_video_rec1 = server_open("video_server", "video1.0");
#else
    ...
    if (!__this_net->net_video_rec1) {
        ...
        sprintf(name, "video3.%d", 1);
#elif (CONFIG_NET_STREAM_DEVICE & CONFIG_UVC1_DEVICE)
        sprintf(name, "video3.%d", 6);
#elif (CONFIG_NET_STREAM_DEVICE & CONFIG_UVC2_DEVICE)
        sprintf(name, "video3.%d", 8);
#elif (CONFIG_NET_STREAM_DEVICE & CONFIG_UVC3_DEVICE)
        sprintf(name, "video3.%X", 0xA);
#endif
...
static void *video0_server[4] = {0};
static void *video0_file[4] = {0};
static int net_video0_state[4];
...
int video0_rec_get_iframe(u8 sub_id)
{
    union video_req req = {0};
    void *server  = get_videox_rec_server(sub_id);
    if (!server) {
        return -1;
    }
...
int ircut_get_isp_scenes_flag(void)
{
    ...
    if (get_videox_rec_server(CONFIG_NET_SUB_ID) && get_videox_rec_state(CONFIG_NET_SUB_ID) == VIDREC_STA_START && isp_scenes_switch_timer_done()) {
   ...
   // } else if (get_videox_rec_server(-1)) {
      //  server = get_videox_rec_server(-1);
        ...
        server = __this_strm->video_rec0;
         printf("\n =========page=============%d,%d, %s\n ",server,__LINE__,__FUNCTION__);
    }else if (get_disp_video()) {
        server = get_disp_video();
         printf("\n =========page=============%d,%d, %s\n ",server,__LINE__,__FUNCTION__);
    }

    if(!server){

       printf("\n =========page server is null=============%d, %s\n ",__LINE__,__FUNCTION__);
       return 0;
    }
...
int net_video_recx_take_photo(u8 sub_id, u8 *buffer, u32 buffer_len)
{
    struct server *server = NULL;
    union video_req req = {0};
    int err;
    ...
    do {
        if (get_videox_rec_state(sub_id) == VIDREC_STA_START) {
            break;
        }
        msleep(10);
    } while (--retry);
    if (retry == 0 && get_videox_rec_state(sub_id) != VIDREC_STA_START) {
        goto exit;
    }

    server = get_videox_rec_server(sub_id);
...
u8 now_msg_fname[MAX_FILE_NAME_LEN]={0};
int user_net_video0_recx_start(u8 sub_id,u8 is_stream)
{
    int err;
    union video_req req = {0};
    struct video_text_osd text_osd;
    ...
    u8 channel = 0;
    u8 format = 0;
    printf("\n =========page=============%d,%d, %s\n ",is_stream,__LINE__,__FUNCTION__);
#ifdef CONFIG_VIDEO0_INIT_ASYNC
    extern int os_query_init_video_sem(void);
    if (os_query_init_video_sem()) {
        printf("\n os_query_init_video_sem err\n");
...
#endif // CONFIG_VIDEO0_INIT_ASYNC
  //  printf("\n >>>>>>>>>%s %d,%d\n",__func__,__LINE__,sub_id);

    puts("start_video_rec0 \n");



    if (!video0_server[sub_id]) {
        if(sub_id == 2){
            video0_server[sub_id] = get_video0_rec_hdl();
        }else{

            char name[12];
            sprintf(name, "video0.%d", sub_id);
            video0_server[sub_id] = server_open("video_server", name);

           //  printf("\n >>>>>>>>>%d,%d,%s %d\n",sub_id,video0_server[sub_id],__func__,__LINE__);
        }

        if (!video0_server[sub_id]) {
            return -EINVAL;
        }
        server_register_event_handler(video0_server[sub_id], (void *)0, net_rec_dev_server_event_handler);
        server = video0_server[sub_id];
        format = 1;
        if (!is_stream) {
            char fname[128] = {0};
            extern uint64_t get_local_msg_start_time(void);
            uint64_t  msg_start_time = get_local_msg_start_time();
            printf("\n\n\nmsg_start_time = %lld \n\n\n",msg_start_time);
            struct tm p = {0};
            extern void covUnixTimeStp2Beijing(uint64_t unixTime,  struct tm *p);
            covUnixTimeStp2Beijing(msg_start_time / (uint64_t)1000, &p);
            if (format) {
                snprintf(fname, sizeof(fname), CONFIG_REC_PATH_1"MSG_%04d%02d%02d_%02d%02d%02d.MOV",p.tm_year, p.tm_mon, p.tm_mday, \
                                                                                    p.tm_hour, p.tm_min, p.tm_sec);
            } else {
                snprintf(fname, sizeof(fname), CONFIG_REC_PATH_1"MSG_%04d%02d%02d_%02d%02d%02d.AVI",p.tm_year, p.tm_mon, p.tm_mday, \
                                                                                    p.tm_hour, p.tm_min, p.tm_sec);
            }
            video0_file[sub_id] = fopen(fname, "w+");
            file = video0_file[sub_id];
#ifdef CONFIG_IPC_UI_ENABLE
            strcpy(now_msg_fname,fname);
            if(now_msg_fname){
                printf("\n/**** now_msg_fname =%s\n",now_msg_fname);
            }
            if(fname){
    ...
    req.rec.qp_attr.static_min = 45;  // 降低数据量
    req.rec.qp_attr.static_max = 48;
    req.rec.qp_attr.dynamic_min = 45;
    req.rec.qp_attr.dynamic_max = 48;
    ...
    if (!is_stream) {
        ...
        req.rec.small_pic_height 	= 96;       //设置缩略图高（与回放窗口一致加快加载速度）
        req.rec.small_pic_width 	= 128;      //设置缩略图宽（与回放窗口一致加快加载速度）
    ...
    net_video0_state[sub_id] = VIDREC_STA_START;

#ifndef CONFIG_IRCUT_ENABLE
    if(sub_id == CONFIG_NET_SUB_ID){
...
int user_net_video0_recx_stop(u8 sub_id,u8 close)
{
    union video_req req = {0};
    int err;
    void *server = NULL;
    ...
    server = video0_server[sub_id];
    file = video0_file[sub_id];
    video0_server[sub_id] = NULL;
    video0_file[sub_id] = NULL;
    channel =  sub_id;

    printf("\n\n >>>>>>>>>>>>>>>>sub_id = %d\n\n",sub_id);
    printf("\n\n >>>>>>>>>>>>>>%s %d\n\n",__func__,__LINE__);
    if(net_video0_state[sub_id] != VIDREC_STA_START){
        return VREC_ERR_V1_REQ_STOP;
    }
    printf("\n\n >>>>>>>>>>>>>>%s %d\n\n",__func__,__LINE__);
    ...
    net_video0_state[sub_id] = VIDREC_STA_STOP;
...
void uvc_h264_frame_decode_end_callback(u8 id, u8 *buf, int len) {
  if (id == 1) {
    static CAL_INFO cal_info = {
        .func = __func__,
        .line = __LINE__,
...
int get_videox_rec_state(u8 sub_id)
{
    int state = 0;
#ifdef CONFIG_VIDEO0_ENABLE
    state =  net_video0_state[sub_id];
#else
    state = uvc_video_state[sub_id];
#endif

    return state;
}

void *get_videox_rec_server(u8 sub_id)
{
    void  *server = NULL;
#ifdef CONFIG_VIDEO0_ENABLE
    server = video0_server[sub_id];
#else
    server = uvc_video_server[sub_id];
#endif
    return server;
}
...
void doorbell_reopen_rt_stream(u8 sub_id)
{
...
#ifdef CONFIG_VIDEO0_ENABLE
    user_net_video0_recx_stop(CONFIG_NET_SUB_ID,0);
    user_net_video0_recx_start(CONFIG_NET_SUB_ID,1);
#else
    if (__this->video_online[3]) {
        user_net_video3_recx_stop(sub_id, 0);
        user_net_video3_recx_start(sub_id, 1);
    }
#endif // CONFIG_VIDEO0_ENABLE
}
...
#ifdef CONFIG_VIDEO0_ENABLE
 //  user_net_video0_recx_start(CONFIG_NET_SUB_ID,0);// 不重新开实时流
    user_net_video0_recx_start(CONFIG_NET_SUB_ID,1);//重新开实时流
#else
...
void doorbell_open_rt_stream(u8 sub_id)
{
    struct application *app = NULL;
    app = get_current_app();
    if (app && strcmp(app->name, "video_rec")) {
...
void doorbell_close_rt_stream(u8 sub_id)
{

#ifdef CONFIG_VIDEO0_ENABLE
    user_net_video0_recx_stop(sub_id,1);
#else
    user_net_video3_recx_stop(sub_id, 1);
#endif // CONFIG_VIDEO0_ENABLE
}

void doorbell_start_msg_rec(void)
{
    struct application *app = NULL;
    app = get_current_app();
    if (app && strcmp(app->name, "video_rec")) {
      ...
      set_local_msg_start_time(get_utc_ms());
      user_net_video0_recx_start(CONFIG_LOCAL_MSG_SUB_ID,0);
#endif // CONFIG_VIDEO0_ENABLE
}
...
#ifdef CONFIG_VIDEO0_ENABLE
      user_net_video0_recx_stop(CONFIG_LOCAL_MSG_SUB_ID,0);
#endif // CONFIG_VIDEO0_ENABLE
...
void doorbell_in_usb(void)
{
   ...
   extern  product_enter_check(void);
   product_enter_check();
...
void doorbell_video_rec_get_iframe(u8 sub_id)
{
#ifdef CONFIG_VIDEO0_ENABLE
    video0_rec_get_iframe(sub_id);
#else
    video3_rec_get_iframe(sub_id);
#endif // CONFIG_VIDEO0_ENABLE
}


int doorbell_video_rec_take_photo(u8 sub_id, u8 *buffer, u32 buffer_len)
{
#ifdef CONFIG_VIDEO0_ENABLE
    return net_video_recx_take_photo(sub_id, buffer, buffer_len);
#else
    return  net_video_recx_take_photo(sub_id, buffer, buffer_len);
#endif // CONFIG_VIDEO0_ENABLE
}
