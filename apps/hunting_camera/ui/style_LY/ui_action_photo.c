...
static int pho_mode_delay5s_poweroff_onchange(void *_ani, enum element_change_event e, void *arg)
{
    ...
    if (e == ON_CHANGE_ANIMATION_END) {
        ...
        ui_hide(LAY_PHO_POWER_OFF);
        ui_hide(PHO_POWER_OFF);
...
REGISTER_UI_EVENT_HANDLER(PHO_POWER_OFF)
.onchange = pho_mode_delay5s_poweroff_onchange,
};

int get_photo_menu_state()
{


    return __this->menu_status;
...
static int tph_headlight_off_handler(const char *type, u32 arg)
{
    if (__this->menu_status == 0) {
        ...
        ui_hide(LAY_PHO_POWER_OFF);
        ui_hide(PHO_POWER_OFF);//hide head light
        __this->hlight_show_status = 0;
    }
    return 0;
}
...
static int tph_switch_rec_handler(const char *type, u32 arg)
{
...
static int tph_returnmenu_handler(const char *type, u32 arg)
{
...
static const struct uimsg_handl tph_msg_handler[] = {
    ...
    { "switch_rec",      tph_switch_rec_handler      },
    { "re_ph",     tph_returnmenu_handler   },
    { NULL, NULL }
};
...
int tph_mode_onkey(void *ctr, struct element_key_event *e)
{
    ...
    printf("\n  ID_WINDOW_VIDEO_TPH e->value:%d,%d\n", e->event, e->value);
    switch (e->event) {
