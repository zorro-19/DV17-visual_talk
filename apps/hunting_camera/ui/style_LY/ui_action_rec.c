...
static int rec_headlight_on_handler(const char *type, u32 arg)
{
    ...
    if (__this->menu_status == 0) {
        ...
        if (__this->hlight_show_status == 0) {
            ...
            ui_show(LAY_REC_POWER_OFF);//show head light
            ui_show(REC_POWER_OFF);//show head light
...
static int rec_headlight_off_handler(const char *type, u32 arg)
{
    if (__this->menu_status == 0) {
        ...
        ui_hide(LAY_REC_POWER_OFF);
        ui_hide(REC_POWER_OFF);//hide head light
        __this->hlight_show_status = 0;
    }
    return 0;
}
...
static int video_mode_delay5s_poweroff_onchange(void *_ani, enum element_change_event e, void *arg)
{


    // printf("\n  rec_e=====================%d\n",e);
    if (e == ON_CHANGE_ANIMATION_END) {
        ui_hide(REC_POWER_OFF);
...
REGISTER_UI_EVENT_HANDLER(REC_POWER_OFF)
.onchange = video_mode_delay5s_poweroff_onchange,
};
...
static int rec_returnmenu_handler(const char *type, u32 arg)
{
