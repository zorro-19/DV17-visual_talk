...
#if  0
static const u16 rec_pix_w[] = { 640, 1280, 1920, 2304, 2560};
static const u16 rec_pix_h[] = {480,   720, 1088, 1296, 1440 };
#else

static const u16 rec_pix_w[] = { 640, 1280, 1920, 2304};
static const u16 rec_pix_h[] = {480,   720, 1088, 1296 };
...
static void rec_dev_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case VIDEO_SERVER_PKG_END:
        ...
        printf("\n rec_dev_server_event_handler_db_select(vtimer):::::::::::::::::::::::::%d\n ", db_select("vtimer"));

        video_rec_stop(0);
        if (switch_work_on_state()) {
            set_workmode_bit(1);// 设置工作完关机标志
        }
        if ((db_select("mode") != 0) && (switch_work_on_state())) {
...
static void video_rec_get_remain_time(void)
{
    ...
    if (storage_device_available()) {
        ...
        //printf("\n  video_rec_get_remain_time_err===========================%d\n",err);
 ...
 */
static u32 video_rec_get_fsize(int cycle_time, u16 vid_width, int format)
{
    u32 fsize;
    ...
    fsize = video_rec_get_abr(vid_width) * cycle_time / 60 * 8250;
...
static int video_rec_del_old_file()
{
...
#endif
    if (cur_space >= (need_space / 1024) * 3 + 250 * 1024) {
        for (i = 0; i < CONFIG_VIDEO_REC_NUM; i++) {
            if (fsize[i] != 0) {
    ...
    while (1) {
        if (cur_space >= (need_space / 1024) * 2 + 250 * 1024) {
            break;
        }
...
static int video0_rec_start()
{
    ...
    printf("\n res::::%d,req.rec.width :::%d,req.rec.height:%d\n", res, req.rec.width, req.rec.height);
    ...
    */
    ...
    struct app_enc_info app_avc_cfg;
#if 0
    app_avc_cfg.pre_pskip_limit_flag = 0;                //SKIP块提前榖Ωِ制开关
    ...
    app_avc_cfg.dsw_size = 0;                            //搜索窗口大小（3x7  1:5x7）
    app_avc_cfg.fs_x_points = 32;                        //水平秴ç¹数
    app_avc_cfg.fs_y_points = 24;                        //垂直秴ç¹数
    app_avc_cfg.f_aq_strength = 0.6f;                    //宏块QP臀åڔ强度
    app_avc_cfg.qp_offset_low = 0;                       //宏块QP槧ܤ؋限
    app_avc_cfg.qp_offset_high = 5;                      //宏块QP槧ܤ؊限
    app_avc_cfg.nr_reduction = 0;                        //DCT降噼eų
    app_avc_cfg.user_cfg_enable = 1;                 //编码参敔ɦȷ配置使能
    req.rec.app_avc_cfg = &app_avc_cfg;
    req.rec.app_avc_cfg = NULL;
#else
    app_avc_cfg.pre_pskip_limit_flag = 1;//0;               //SKIP块提前判断限制开关
    app_avc_cfg.pre_pskip_th = 0;                       //SKIP块提前判断阈值
    app_avc_cfg.common_pskip_limit_flag = 1;//0;            //SKIP块常规判断限制开关
    app_avc_cfg.common_pskip_th = 0;                    //SKIP块常规判断阈值
    app_avc_cfg.dsw_size = 1;                           //搜索窗口大小（0:3x7  1:5x7）
    app_avc_cfg.fs_x_points = 32;                       //水平搜索点数
    app_avc_cfg.fs_y_points = 24;                       //垂直搜索点数
    app_avc_cfg.f_aq_strength = 0.1f;//0.3f;                    //宏块QP自适应强度
    app_avc_cfg.qp_offset_low = -5;//0;                     //宏块QP偏移下限
    app_avc_cfg.qp_offset_high = 1;//5;                     //宏块QP偏移上限
    app_avc_cfg.nr_reduction = 1000;//5000;                     //DCT降噪开关
    app_avc_cfg.user_cfg_enable = 1;                    //编码参数用户配置使能
    req.rec.app_avc_cfg = &app_avc_cfg;
    ...
    if (req.rec.camera_type != VIDEO_CAMERA_UVC) {
        ...
        req.rec.tlp_time = 0; //1000;// db_select("gap");
        if (req.rec.tlp_time) {
    ...
    req.rec.cycle_time = db_select("vtimer");


    printf("\n req.rec.cycle_time====%d,db_select(cyc):%d.vtimer:%d\n", req.rec.cycle_time, db_select("cyc"), db_select("vtimer"));
#if 1
    if (req.rec.cycle_time == 0) {
        req.rec.cycle_time = 10;
    }
...
int video0_rec_gap_start()
{
    ...
    req.rec.channel     = 0;
    req.rec.camera_type = VIDEO_CAMERA_NORMAL;
    req.rec.width 	    = 1920; // rec_pix_w[res];
    req.rec.height 	    = 1088; // rec_pix_h[res];
    printf("\n res::::%d,req.rec.width :::%d,req.rec.height:%d\n", res, req.rec.width, req.rec.height);
    ...
    */
    ...
    printf("\n gap>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
    if (req.rec.camera_type != VIDEO_CAMERA_UVC) {
        req.rec.tlp_time = 1000; // db_select("gap");
        if (req.rec.tlp_time) {
            req.rec.real_fps = 1000 / req.rec.tlp_time;
            req.rec.real_fps = 1;
            req.rec.pkg_fps	 = 5;
...
extern u32 photo_db_select(const char *table);
static int video_rec_take_photo(u8 id)
{
    ...
    req.icap.width = 5504;//rec_pix_w[VIDEO_RES_720P];
    req.icap.height = 2944;//rec_pix_h[VIDEO_RES_720P];
    req.icap.quality = VIDEO_MID_Q;
    //req.icap.text_label = NULL;


    if (req.icap.width > 1920 && !__this->aux_buf) {
        /*
         *设置尺寸较大时缩放所需要的buffer
         */
    ...
    req.icap.src_w = __this->src_width[id];
    req.icap.src_h = __this->src_height[id];


    req.icap.graph_label = NULL;//&graph_osd;

    req.icap.text_label = photo_db_select("pdat") ? &__this->label : NULL;
    set_label_config(id, req.icap.width, req.icap.height, 0xe20095, req.icap.text_label);
...
static int video_rec_start_isp_scenes()
{
    ...
    //printf("\n __this->video_display[0]:%d,__this->video_rec0:%d,__this->state:%d\n ",__this->video_display[0],__this->video_rec0,__this->state);
    if (__this->video_display[0]) {
        __this->isp_scenes_status = 1;
        return start_update_isp_scenes(__this->video_display[0]);
    } else if (__this->video_rec0 && ((__this->state == VIDREC_STA_START) ||
                                      (__this->state == VIDREC_STA_STARTING) || (__this->state == VIDREC_STA_IDLE))) {
...
static int video_rec_storage_device_ready(void *p)
{
    ...
    if (gpio_read(MENU_ON_SET_GPIO)) {
        ...
        if ((int)p == 1) {
            ...
            //video0_rec_gap_start();
            //   sys_timeout_add(NULL, video_rec_stop, 1000); //时间到停止写卡
#else
            /* video_rec_start(); */
#endif
...
void  switch_to_photo_mode()
{


    printf("\n switch_photo>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>delay_pv:%d\n ", db_select("delay_pv"));



    printf("\n tph_switch_rec_handler>>>>>>>>>>>>>\n");
    if (db_select("delay_pv")) { // 拍完照 切到录像模式 录像
        struct intent it;
...
static int pir_video_rec_stop(u8 close)
{
    int err;
    __this->need_restart_rec = 0;
    ...
    puts(" pir video_rec_stop: exit\n");


    db_update("key_state", 0); // 清除m, 另启动文件标志
    db_flush();
...
static int pir_video_rec_storage_device_ready(void *p)
{
    __this->sd_wait = 0;
    ...
    if (gpio_read(MENU_ON_SET_GPIO)) {
        ...
        printf("\n pir_video_rec_storage_device_ready\n  ");
        if ((int)p == 1) {
#if (APP_CASE == __WIFI_CAR_CAMERA__)
            // video_rec_start_notify();//先停止网络实时流再录像,录像完毕再通知APP
            create_jpeg_video_file();
            sys_timeout_add(NULL, pir_video_rec_stop, 1000); //时间到停止写卡
#else
            /* video_rec_start(); */
#endif
...
static int pir_video_rec_del_old_file()
{
    ...
    u32 cur_space;
    u32 need_space = 0;
    u32 gap_time = 1000; // db_select("gap");
    int cyc_time = db_select("vtimer");
...
static int  create_jpeg_video_file()
{
     ...
     * 判断SD卡空间，删除旧文件并创建新文件
     */
    err = pir_video_rec_del_old_file();
    if (err) {
        log_e("start free space err\n");
    ...
    log_d("cretate video_rec_start: out )))))))\n");
    /* malloc_dump(); */
    malloc_stats();
...
static int video_rec_init_on_disp()
{
    int err = 0;
...
static int video_rec_state_machine(struct application *app, enum app_state state, struct intent *it)
{
    ...
    case APP_STA_START:
        ...
        case ACTION_AUTO_VIDEO_REC_DISP:  // 切模式只做显示用

            puts("ACTION_AUTO_VIDEO_REC_START\n");
            video_rec_init_on_disp();
            break;
        case ACTION_PIR_VIDEO_REC:  // 启动录制一个1帧封装视频

            puts("ACTION_PIR_VIDEO_REC_START\n");


            __this->sd_wait = wait_completion(storage_device_ready,
                                              pir_video_rec_storage_device_ready, (void *)1);
        ...
        case ACTION_VIDEO_REC_SET_CONFIG:
            ...
            if (it->data && !strcmp(it->data, "vtimer")) {
                video_rec_len();
            }
            break;
        ...
        case DEVICE_EVENT_POWER_CHARGER_IN:
            ...
            if ((__this->state == VIDREC_STA_IDLE) ||
                ...
                if (__this->char_wait == 0) {
                    ...
                    __this->char_wait = wait_completion(storage_device_ready,
                                                        video0_rec_gap_start, (void *)0);


                    sys_timeout_add(NULL, video_rec_stop, 1000); //时间到停止写卡
                    // sys_timeout_add(NULL, video_rec_stop, 30*1000); //时间到停止写卡
