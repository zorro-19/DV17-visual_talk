...
static volatile u8 gsen_active_flag = 0;
static volatile u8 gsen_active_state = 0;
/**
...
u8 get_gsen_active_state()
{
    return !!gsen_active_state;
}
    ...
    if (par) {
        ...
        if (!ret) {
            ...
            if (gsen_active_flag == 0) {
                gsen_active_flag = 1;
                gsen_active_state = 10;
                send_key_lock_file_msg();
            }
        } else {
            if (gsen_active_state) {
                gsen_active_state--;
            }
            gsen_active_flag = 0;
        }
...
static int gsensor_init()
{
    ...
    char wkup_reason[30];
    sys_power_get_wakeup_reason(wkup_reason, sizeof(wkup_reason));
    if (!strcmp(wkup_reason, "wkup_port:wkup_gsen")) {
        set_park_guard(1);
    } else {
        set_park_guard(0);
    }
