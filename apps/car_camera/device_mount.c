...
int mount_sd_to_fs(const char *name)
{
...
#if 0
    {
        ...
        u32 cur_space;
        u32 used_space;
        dev_ioctl(fd, SD_IOCTL_GET_CID, (u32)&cid);
        printf("MID=0x%x\n", cid.MID);
        printf("OID=%s\n", cid.OID);
        printf("PNM=%s\n", cid.PNM);
        printf("PSN=%x\n", cid.PSN);
        ...
        dev_ioctl(fd, SD_IOCTL_GET_CLASS, (u32)&type);
        /* UNKNOWN = 0,                             ///<不支持 */
        /* MMC_CARD,                                ///<MMC */
        /* MMC_V4,                                  ///<MMC 4.0 or higher */
        /* SD_V1,                                   ///<SD 1.0 */
        /* SD_V2,                                   ///<SD 2.0 */
        /* SD_HC,                                   ///<SDHC 2.0 */
        printf("TYPE=%d\n", type);
        ...
        log_d("total_size:%d\n", part->total_size); //Byte
        fget_free_space(CONFIG_ROOT_PATH, &cur_space); //Byte
        printf("space: %d\n", cur_space);
        printf("used_space: %d\n", part->total_size - cur_space);
    }
#endif
    dev_close(fd);
__err:
