...
#ifdef CONFIG_IQ_DEBUG_ENABLE
#define UPDATE_UVC_ISP_SCENES    0
#else
#define UPDATE_UVC_ISP_SCENES    1
#endif
#define UVC_CUSTOM_UPGRADE_EN    1   //uvc升级功能,需要搭配pc上位机
...
static int uvc_processing_unit_response(struct uvc_unit_ctrl *ctl_req)
{
    ...
    if (ctl_req->unit == 0x0a) {//white_blance
        ...
        if (ctl_req->unit == 0x01 && ctl_req->request == 0x01) {
            static struct usb_video_cmd_t _cmd;
            memcpy(&_cmd, ctl_req->data, 4);
            /* printf("id: %d\n", _cmd.id);  */
            /* printf("cmd: %d\n", _cmd.cmd);  */
            /* printf("data: %d\n", _cmd.data);  */
...
static int uvc_extension_unit_response(struct uvc_unit_ctrl *ctl_req)
{
    int err = 0;
    //自定义命令
    enum {
    ...
    const char sensor_name[10];
    const char *no_camera = "no camera";
    if (ctl_req->request == 0x81) {
        //GET_CUR
        /* log_d("uvc_user_extension_get [%d]:", ctl_req->wValue); */
        switch (ctl_req->unit) {
        case CMD_UVC_EXT_CAM_NAME: {
            log_d("CMD_UVC_EXT_CAM_NAME");
            void *dev = dev_open("video0.0", NULL);
            if (dev) {
                dev_ioctl(dev, CAMERA_GET_SENSOR_ID, (u32)sensor_name);
                dev_close(dev);
                strcpy((char *)ctl_req->data, sensor_name);
                ctl_req->len = strlen(sensor_name);
            } else {
                strcpy((char *)ctl_req->data, no_camera);
                ctl_req->len = strlen(no_camera);
            }
            return 0; //接管
        }
        case CMD_UVC_EXT_FW_VERSION: {
            log_d("CMD_UVC_EXT_FW_VERSION");
            const char *fw_version = "AC57N_SDK_V3.0.3";
            strcpy((char *)ctl_req->data, fw_version);
            ctl_req->len = strlen(fw_version);  //不能超过64Byte
            return 0; //接管
        }
        }
    }
    if (ctl_req->request == 0x01) {
        //SET_CUR
    }
...
void user_uvc_camera_before_open(u16 width, u16 height)
{
    extern int os_query_init_video_sem(void);
    os_query_init_video_sem();
}
void user_uvc_camera_set_crop(void *fd, u16 width, u16 height)
{
    ...
    int err;
    static void *uvc_fd2 = NULL;
    static u8  uvc_open = 0;
    const char *video_name = "video0.2";
    if (width && height) {
        log_d(">>>> user_uvc_camera %d x %d\n", width, height);
        uvc_open++;
        if (!uvc_fd2) {
            uvc_fd2 = dev_open(video_name, NULL);
            if (uvc_fd2) {

                log_d("open %s ok\n", video_name);
                breq.size = 2 * 1024 * 1024;
                err = dev_ioctl(uvc_fd2, VIDIOC_REQDBUF, (unsigned int)&breq);
                f.type = VIDEO_BUF_TYPE_UVC_CAPTURE;
                f.pixelformat = VIDEO_PIX_FMT_RAW | VIDEO_PIX_FMT_YUV420;
                if (fd) {
                    //默认尺寸
                    f.win.width = 480;
                    f.win.height = 320;
                ...
                f.fps = 22;
                dev_ioctl(uvc_fd2, VIDIOC_SET_FMT, (u32)&f);
            }
        }
    }
    if (width == 0 && height == 0) {
...
/**
 * @brief  uvc 抓拍接口,从机调用该接口会触发amcap等上位机拍照
 * @param: none
 * @return:none
 **/
void uvc_shutter(void *p)
{
    extern void usb_interrupt_shutter(void);
    printf("\n[ debug ]--func=%s line=%d\n", __func__, __LINE__);
    usb_interrupt_shutter();
}
/*
 *设置PC Camera
 */
int set_usb_camera(void)
{
...
#if (defined CONFIG_UVC_TWO_H264_ENABLE)
    //UVC_TWO_H264_OUTPUT config
    req.camera[0].enable = 1;
    req.camera[0].format = USB_VIDEO_FMT_H264;
    req.camera[0].name = "usb_cam0"; //device 设备名，需要在板级文件中定义
    req.camera[0].video_open = usb_video_h264_open; //h264 视频流open回调
    req.camera[0].video_close = usb_video_h264_close; //h264 视频流close回调
    ...
    /* req.camera[0].rotate = UVC_ENC_R90; //开旋转比较占据内存，特别是1080p旋转，开双路h264要适当减少大的内存申请 */

    req.camera[1].enable = 1; //使能开关
    req.camera[1].split_cam_enable = 2; //

#elif (defined CONFIG_UVC_ONE_H264_ENABLE)
    //usb camera0 为MJPEG/YUYV视频流定义
    req.camera[0].enable = 0;
...
#elif (defined CONFIG_UVC_ONE_JPEG_ENABLE)
    //usb camera0 为MJPEG/YUYV视频流定义
    req.camera[0].enable = 1;
    req.camera[0].format = USB_VIDEO_FMT_MJPG;// | USB_VIDEO_FMT_YUY2;
    req.camera[0].buf = __this->buf;
    ...
    req.camera[1].split_cam_enable = 0; // 表示是否分离出第2个uvc设备,双码流UVC设备 1:分离出1个h264码流 2:分离出2个h264码流

#elif (defined CONFIG_UVC_JPEG_H264_ENABLE)
    ...
    //usb camera1 为H264视频流定义
    req.camera[1].enable = 1; //使能开关
    req.camera[1].split_cam_enable = 1; // 表示是否分离出第2个uvc设备,双码流UVC设备 1:分离出1个h264码流 2:分离出2个h264码流
    ...
    req.camera[0].extension_unit_response = uvc_extension_unit_response;
#endif

    req.camera[1].name = "usb_cam1"; //device 设备名，需要在板级文件中定义
    req.camera[1].info = &uvc_h264_reso_info;
    req.camera[1].format = USB_VIDEO_FMT_H264;
    req.camera[1].video_open = usb_video_h264_open2; //h264 视频流open回调
    req.camera[1].video_close = usb_video_h264_close2; //h264 视频流close回调
    req.camera[1].rotate = UVC_ENC_R0;


#if (defined CONFIG_UVC_ONE_H264_ENABLE)
    req.camera[1].name = "usb_cam0";
    req.camera[1].video_open = usb_video_h264_open;
    req.camera[1].video_close = usb_video_h264_close;
#endif
#ifdef PRODUCT_TEST_ENABLE
    ...
    /* sys_timer_add(NULL,uvc_shutter,10000); */
...
/**
 * @brief   uvc在任务里请求视频帧弱定义函数
 *
 * @param:  usb slave 句柄
 *
 * @return: 1:发送消息到任务处理
 *          0:不发送消息到任务处理,由弱定义函数里进行处理
 **/
//从机打开，加快数据发送，防止出现花屏
int uvc_req_buf_in_task(void *slave, u32 ep)
{
#ifdef CONFIG_UVC_SLAVE_ENABLE
//    printf("\n ep = %d\n",ep);
    if (slave) {
        if (ep == USB_EP4) {
//            putchar('A');
            extern void uvc_g_video_transfer(void *slave);
            uvc_g_video_transfer(slave);
        }

        if (ep == USB_EP2) {
//            putchar('B');
            void uvc_g_video_transfer_h264(void *slave);
            uvc_g_video_transfer_h264(slave);
        }
