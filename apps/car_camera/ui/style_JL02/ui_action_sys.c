...
static int set_format_reset_func(int p)
{
    btn_warning_show_f = 0;
...
static void sys_menu_show(enum SYS_MENU item)
{
    ...
    case SYS_MENU_VERSION:
        ...
        /* >>>>>>>>>>>>显示版本号 */
        sprintf(__this->version, "VERSION: 2.0.0 beta");
#ifdef PRODUCT_TEST_ENABLE
        int product_read_options_string(char *key, char *string);
        if (product_read_options_string("ver", __this->version) == 0) {
            printf("read options version info OK");
        }
#endif
        ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", __this->version);
        break;
    default:
        break;
    }
