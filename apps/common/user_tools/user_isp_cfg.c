...
#define USER_ISP_CFG_NIGHT_H    ((1 << 8) 	+ 100)

#define USER_ISP_CFG_INDOOR_L   ((1 << 8) 	+ 128)
#define USER_ISP_CFG_INDOOR_H   ((6 << 8) 	- 0)
...
unsigned char wdr_en[] = {0, 0, 0, 0};/* 0:该场								 1:该场景开wdr(菜单在开启wdr使能时，该场景的wdr才开)*/
...
#if (defined CONFIG_UI_ENABLE && !defined CONFIG_IPC_UI_ENABLE)
...
int stop_update_isp_scenes(void)
{
    if (__this->timer) {
        ...
        do{
            if(isp_scenes_switch_timer_done()){
                break;
            }
            msleep(30);
        }while(1);
...
int start_update_isp_scenes(struct server *server)
{
 ...
 printf("\n ==============page=============%d,%s\n ",__LINE__,__FUNCTION__);
    __this->timer =  sys_timer_add_to_task("sys_timer",server, isp_scene_switch_timer, 50);
    return 0;
}
