...
static void ctp_cli_recv_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (!strncmp(topic_data, hdl->topic, 19)) {
            /* printf("####  content_data:%s  content_len:%d\n", content_data, content_len); */
            sscanf(content_data, "{\"op\":\"NOTIFY\",\"param\":{\"timeout\":\"%d", &hdl->timecheckout);
            hdl->timeout = 0.8 * hdl->timecheckout ;
        }
...
static int ctp_sock_cb(enum sock_api_msg_type type, void *priv)
{
    ...
    case SOCK_RECV_TO:
        //time_lapse
        if ((hdl->timeout != 0) && (time_lapse(&hdl->timehdl, hdl->timeout))) {
            if (ctp_cli_send(hdl, hdl->topic, &Get_data) != 0) {
        ...
        } else if ((hdl->timeout == 0) && (time_lapse(&hdl->timecheck, hdl->timecheckout))) {
            ret = -1;
            hdl->state = SERVER_CLOSE;
            return ret;
        }
...
void *ctp_cli_reg(u16_t id, struct sockaddr_in *dest_addr, int (*cb_func)(void *hdl, enum ctp_cli_msg_type type, const char *topic, const char *parm_list, void *priv), void *priv, char *topic, u16_t timeout)
{
    struct ctp_hdl *hdl;
