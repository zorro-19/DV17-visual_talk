...
static void get_yuv_task(void *priv)
{
    ...
    const char *dev_name = "video0.3";
    const char *fb_name = "fb3";
...
void get_yuv_init2(void (*cb)(u8 *data))
{
    ...
    __info.cb = cb;
    __info.exit_state = 0;
    thread_fork("GET_YUV_TASK0", 10, 0x1000, 0, &__info.pid, get_yuv_task, NULL);
}
void get_yuv_uninit2(void)
{
