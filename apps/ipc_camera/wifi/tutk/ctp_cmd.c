...
static inline int _CTP_CMD_COMBINED(int cmd_type, void *priv, u32 err, const char *_req, const char *mothod, char *str)
{
    ...
    if (!err && str != NULL) {
        while (1) {
            ...
            if (value) {
                ...
                ret = sprintf(tmp, "\"%s\":null,", key, value);
            }
            tmp[ret] = '\0';
    ...
    printf("ctp send buf->%s\n", buf);

    if (send_ctp_string(cmd_type, buf, _req, priv) < 0) {
        free(buf);
        return -1;
    }
...
#elif defined CONFIG_UI_STYLE_JL02_ENABLE
    video_rec_post_msg("setRec");
#endif
#endif
    ...
    if (storage_device_ready() == 0) {
        ...
        if (!strcmp(app->name, "video_rec")) {
            ...
            if (net_handler->net_state == VIDREC_STA_START
                ...
                extern int fv_video_rec_take_photo();
                fv_video_rec_take_photo();
...
static int cmd_put_close_pull_rt_stream(void *priv, char *content)
{
...
static int cmd_get_sta_ssid_info(void *priv, char *content)
{
    ...
    if (strlen(info.pwd) >= 8) {
        sprintf(buf, "ssid:%s,pwd:%s", info.ssid, info.pwd);
    } else {
        sprintf(buf, "ssid:%s", info.ssid);
    }
...
{
    printf("cmd_put_open_rt_stream\n");
    json_object *new_obj = NULL;
    json_object *parm = NULL;
    char buf[256];
    int ret = 0;
    const char *h = NULL, *w = NULL, *format = NULL, *fps = NULL, *rate = NULL;
    new_obj = json_tokener_parse(content);
    parm =  json_object_object_get(new_obj, "param");
    h = json_object_get_string(json_object_object_get(parm, "h"));
    w = json_object_get_string(json_object_object_get(parm, "w"));
    ...
    printf("parm:%x h:%x w:%x format:%x fps:%x  rate:%x\n", parm, h, w, format, fps, rate);

    u32 width, height, fps1, rate1, format1;
    ...
    if (rate) {
        ...
        rate1 = 8000;//不写默认8000采样率
    }


    if (format && atoi(format) == 1) {
        format1  = VIDEO_FMT_MOV;
    } else if (format && atoi(format) == 0) {

        format1 = VIDEO_FMT_AVI;
    } else {
    ...
    printf("width:%d  height:%d fps:%d  rate:%d  format:%d\n", width, height, fps1, rate1, format1);

    ret = video0_tutk_start(priv, width, height, fps1, rate1, format1);
    if (ret < 0) {
        /* video0_tutk_stop(priv); */
        CTP_CMD_COMBINED(priv, CTP_RT_OPEN_FAIL, "OPEN_RT_STREAM", "NOTIFY", CTP_RT_OPEN_FAIL_MSG);
    } else {

        int len = sprintf(buf, "format:%d,w:%d,h:%d,fps:%d,rate:%d", format1 == VIDEO_FMT_MOV ? 1 : 0, width, height, fps1, rate1);

        printf("buf=>%s len:%d\n", buf, len);
        CTP_CMD_COMBINED(priv, CTP_NO_ERR, "OPEN_RT_STREAM", "NOTIFY", buf);
...
//设置参数
    ...
    if (atoi(status)) {
        ret = video0_tutk_stop(priv);
        if (ret < 0) {
            CTP_CMD_COMBINED(priv, CTP_REQUEST, "CLOSE_RT_STREAM", "NOTIFY", CTP_REQUEST_MSG);
        } else {
    ...
    printf("cmd_put_close_rt_stream\n");
...
int cmd_put_open_speak_stream(void *priv, char *content)
{
    ...
    ret = tutk_speak_init(priv, format1, rate1);
    /* ret = video0_tutk_start(priv, rate1, format1); */
    if (ret < 0) {
...
static int cmd_put_make_forward_files_list(void *priv, char *content)
{
    ...
    case VID_JPG:
        if (!file_num) {
            int len = FILE_GEN_SYNC(CONFIG_REC_PATH_0"vf_list.txt", CONFIG_REC_PATH_0);
            if (len > 0) {
                sprintf(buf, "type:1,path:%s,size:%d", CONFIG_REC_PATH_0"vf_list.txt", len);
            ...
            CTP_CMD_COMBINED(priv, CTP_SET_PRARM, "FORWARD_MEDIA_FILES_LIST", "NOTIFY", CTP_SET_PRARM_MSG);
...
static int cmd_put_make_behind_files_list(void *priv, char *content)
{
    ...
    case VID_JPG:
        if (!file_num) {
            int len = FILE_GEN_SYNC(CONFIG_REC_PATH_1"vf_list.txt", CONFIG_REC_PATH_1);
            if (len > 0) {
                sprintf(buf, "type:1,path:%s,size:%d", CONFIG_REC_PATH_1"vf_list.txt", len);
            ...
            CTP_CMD_COMBINED(priv, CTP_SET_PRARM, "BEHIND_MEDIA_FILES_LIST", "NOTIFY", CTP_SET_PRARM_MSG);
...
#define VERSION1 "JL-IPC ver:20190801XY"
int cmd_get_version(void *priv, char *content)
{
    char buf[32];

    sprintf(buf, "version:%s", VERSION1);
    CTP_CMD_COMBINED(priv, CTP_NO_ERR, "VERSION", "NOTIFY", buf);
    return 0;

}


#if defined CONFIG_STEP_MOTOR_ENABLE
#include "step_motor.h"
static int cmd_put_device_directionB(void *priv, char *content)
{
    extern struct timer_moto_port timer_moto1;

    //左右
    json_object *new_obj = NULL;
    json_object *parm = NULL;
    json_object *dire = NULL;
    json_object *role = NULL;
    char buf[128];
    new_obj = json_tokener_parse(content);
    parm =  json_object_object_get(new_obj, "param");
    dire =  json_object_object_get(parm, "dire");
    const char *dire_value = json_object_get_string(dire);
    printf(">>>BBBBBdevice_direction:%s\n", dire_value);


    role =  json_object_object_get(parm, "role");
    const char *role_value = json_object_get_string(role);
    printf(">>>BBBBBdevice_role:%s\n", role_value);

    if (!dire_value || !role_value) {

        CTP_CMD_COMBINED(NULL, CTP_REQUEST, "DEVICE_DIRECTION_CONTROLB", "NOTIFY", CTP_REQUEST_MSG);
    }


    if (atoi(dire_value) == 1) { //异步的，自己看着吧，加延时
        step_moto_timer_ctrl(STEP_MOTO_ROTA_ANTICLOCK, &timer_moto1, 550, atoi(role_value), 2);
        msleep(500);
    } else {
        step_moto_timer_ctrl(STEP_MOTO_ROTA_CLOCKWISE, &timer_moto1, 550, atoi(role_value), 2);
        msleep(500);
    }



    sprintf(buf, "dire:%s,role:%s", dire_value, role_value);
    CTP_CMD_COMBINED(NULL, CTP_NO_ERR, "DEVICE_DIRECTION_CONTROLB", "NOTIFY", buf);
...
static int cmd_put_device_directionA(void *priv, char *content)
{
    extern struct timer_moto_port timer_moto2;
    ...
    dire =  json_object_object_get(parm, "dire");
    const char *dire_value = json_object_get_string(dire);
    printf(">>>AAAAAAdevice_direction:%s\n", dire_value);


    role =  json_object_object_get(parm, "role");
    const char *role_value = json_object_get_string(role);
    printf(">>>AAAAAAdevice_role:%s\n", role_value);
    ...
    if (atoi(dire_value) == 1) { //异步的，自己看着吧，加延时
        step_moto_timer_ctrl(STEP_MOTO_ROTA_ANTICLOCK, &timer_moto2, 550, atoi(role_value), 2);
        msleep(500);
    } else {
        step_moto_timer_ctrl(STEP_MOTO_ROTA_CLOCKWISE, &timer_moto2, 550, atoi(role_value), 2);
        msleep(500);
    }
    ...
    sprintf(buf, "dire:%s,role:%s", dire_value, role_value);
    CTP_CMD_COMBINED(NULL, CTP_NO_ERR, "DEVICE_DIRECTION_CONTROLA", "NOTIFY", buf);
...
int cmd_put_video_quality(void *priv, char *content)
{
    json_object *new_obj = NULL;
    json_object *parm = NULL;
    json_object *tmp = NULL;
    ...
    new_obj = json_tokener_parse(content);
    parm =  json_object_object_get(new_obj, "param");
    tmp =  json_object_object_get(parm, "vqua");

    const char *tmp_value = json_object_get_string(tmp);
    printf("ctp pro  %s \n", tmp_value);
    db_update("vqua", atoi(tmp_value));

    ret = video0_tutk_change_bits_rate(priv, atoi(tmp_value));
    if (ret < 0) {

        CTP_CMD_COMBINED(priv, CTP_REQUEST, "VIDEO_QUALITY", "NOTIFY", CTP_REQUEST_MSG);

    } else {
        sprintf(buf, "vqua:%d", db_select("vqua"));
        CTP_CMD_COMBINED(priv, CTP_NO_ERR, "VIDEO_QUALITY", "NOTIFY", buf);
...
int cmd_get_video_quality(void *priv, char *content)
{
    char buf[64];
    u32 vqua = 0;
    sprintf(buf, "vqua:%d", db_select("vqua"));
...
static int cmd_put_ap_ssid_info(void *priv, char *content)
{
    ...
    printf("ssid:%s len:%d  pwd:%s \n", ssid, strlen(ssid), pwd);
    if (strlen(pwd) == 0) {
...
static int cmd_put_smp_ssid_info(void *priv, char *content)
{
    char buf[64];

    sprintf(buf, "status:%d", 1);
    CTP_CMD_COMBINED(priv, CTP_NO_ERR, "SMP_SSID_INFO", "NOTIFY", buf);
...
static int cmd_get_wifi_mode_info(void *priv, char *content)
{
    char buf[64];

    sprintf(buf, "mode:%d", get_cur_wifi_info()->mode);

    CTP_CMD_COMBINED(priv, CTP_NO_ERR, "WIFI_MODE_INFO", "NOTIFY", buf);
...
const struct ctp_map_entry ctp_video_cmd_tab[] sec(.ctp_video_cmd) = {
    ...
    {NULL, "VIDEO_QUALITY", cmd_get_video_quality, cmd_put_video_quality},
...
const struct ctp_map_entry ctp_system_cmd_tab[] sec(.ctp_system_cmd) = {
    ...
    {NULL, "VERSION", cmd_get_version, NULL},
    {"kep", "KEEP_ALIVE_INTERVAL", cmd_get_keep_alive_interval, NULL},
    {NULL, "AP_SSID_INFO", cmd_get_ap_ssid_info, cmd_put_ap_ssid_info},
    {NULL, "SMP_SSID_INFO", NULL, cmd_put_smp_ssid_info},
    {NULL, "WIFI_MODE_INFO", cmd_get_wifi_mode_info, NULL},
#if defined CONFIG_STEP_MOTOR_ENABLE
    {NULL, "DEVICE_DIRECTION_CONTROLA", NULL, cmd_put_device_directionA}, //控制设备方向
    {NULL, "DEVICE_DIRECTION_CONTROLB", NULL, cmd_put_device_directionB}, //控制设备方向
#endif
#if 0
