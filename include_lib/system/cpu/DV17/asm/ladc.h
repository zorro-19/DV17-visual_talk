...
#define LADC_IE_EN()           		   (ADC_CON |= BIT(5))
#define LADC_IE_DIS()           	   (ADC_CON &= ~BIT(5))
...
int dv17_adc_open(int sample_rate, int channel, u8 ldo_sel);


int dv17_adc_set_sample_rate(int sample_rate);
...
void dv17_adc_set_data_handler(void *priv, void (*handler)(void *, u8 *data, int len));

int dv17_adc_set_gain(u8 gain_val);
