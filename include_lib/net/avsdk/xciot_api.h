...
/**
 * @brief 申请复用视频static的内存
 * 1.该内存大小，为avsdk_init传入的大小，特别注意越界.
 * 2.不再使用该内存时，需要调用avsdk_release_mux_static_buf，将buf使用权释放掉
 *
 * @return 申请成功返回地址指针，失败返回NULL
 */
void *avsdk_apply_mux_static_buf(void);

/**
 * @brief 释放复用视频static的内存
 *
 * @return 无
 */
void avsdk_release_mux_static_buf(void);
...
/**
 * @brief 发送视频媒体数据，无缓存，适合内存很小(512k)的设备.
 *
 * @param[in] streamtype		- 音视频数据类型 @ref iot_mediastream_type_t. 
 *
 * @param[in] iskeyframe		- 是不是关键帧(i frame set true or false).
...
void avsdk_write_video_data_nocache(iot_mediastream_type_t stream_type,
                            ...
                            uint32_t               payload_len);
/**
 * @brief 发送视频媒体数据，实时和回看都可以，无缓存，适合内存很小(512k)的设备..
 *
 * @param[in] conn_id         - 连接ID.
 * @param[in] streamtype      - 音视频数据类型 @ref iot_mediastream_type_t. 
 * @param[in] iskeyframe      - 是不是关键帧(i frame set true or false). 
 ...
 */
int avsdk_write_media_nocache(int32_t                conn_id,
                         iot_mediastream_type_t streamtype,
                         bool                   iskeyframe,
                         uint64_t               avchannel,
                         iot_encrypt_type_t     enctype,
                         ...
                         size_t                 mediadatalength);
/**
 * @brief 发送视频媒体数据，使用固定缓存，适合内存很小(512k)的设备.
 *
 * @param[in] streamtype		- 音视频数据类型 @ref iot_mediastream_type_t. 
 *
 * @param[in] iskeyframe		- 是不是关键帧(i frame set true or false).
 ...
 * @return 无.
 */
void avsdk_write_video_static(iot_mediastream_type_t stream_type,
                               bool                   is_key_frame,
                               uint64_t               avchannel,
                               uint64_t               timestamp_ms,
                               ipc_AIDesc            *AIDesc,
 ...
 * @return 无.
 */
void avsdk_write_video_data_static(iot_mediastream_type_t stream_type,
                            bool                   is_key_frame,
                            uint64_t               avchannel,
                            iot_encrypt_type_t     encrypt_type,
                            uint64_t               timestamp_ms,
...
/**
 * @brief 发送视频媒体数据，实时和回看都可以，使用固定缓存，适合内存很小(512k)的设备..
 *
 * @param[in] conn_id         - 连接ID.
 * @param[in] streamtype      - 音视频数据类型 @ref iot_mediastream_type_t. 
 * @param[in] iskeyframe      - 是不是关键帧(i frame set true or false). 
 ...
 * @return 0成功，-9 conn_id已断开，需要退出sd发送，其他返回值可以暂时不处理.
 */
int avsdk_write_media_static(int32_t                conn_id,
                         iot_mediastream_type_t streamtype,
                         bool                   iskeyframe,
                         uint64_t               avchannel,
                         iot_encrypt_type_t     enctype,
