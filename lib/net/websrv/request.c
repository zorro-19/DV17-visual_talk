...
static int cgi_handler(WrHttp *pHttp ,void *handle)
{
...
static int sendHead(const WrHttp *pHttp ,char *pBuf ,size_t len)
{
	...
	return wr_sock_nwrite(pHttp->sockhdl ,pBuf ,len+nLen);
}
...
int wr_send_msg(WrHttp *pHttp ,const char *type ,const char *buf ,size_t len)
{
	...
	do{
		if((ret=sendHead(pHttp ,hBuf ,pBuf-hBuf)) <0)
			break;
		if(ret=wr_sock_nwrite(pHttp->sockhdl ,buf ,len) <0)
			break;
	} while(0);
	return ret;
}
...
int wr_send_file(WrHttp *pHttp ,const char *filePath)
{
	...
	virtual_stat(filePath ,&pHttp->st);
...
static int cgiProcess(WrHttp *pHttp)
{
	...
	so we must free it*/
	if(*(pHttp->method)=='P' && pHttp->postData!=pHttp->curPos)
	{
	    free(pHttp->postData);
	}
...
void requestHandler(struct CliContext *cli)
{
	int ret = -1;
	...
	do{
		...
		httpInfo.recvLen =wr_read_head(cli->sock_hdl ,recvBuf ,MAX_HEADER_SIZE);
...
//		websrv_print_header(&httpInfo);
	...
	websrv_disconnect_cli(cli->ipaddr);
}
