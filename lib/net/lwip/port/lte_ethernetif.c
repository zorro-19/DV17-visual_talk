...
static int create_eth_rx_task(void *p_arg, u8 prio)
{
    ...
    return os_task_create(lte_rx_task, (void *)p_arg, prio, 0x2000, 0, "lte_rx_task_n");
}

static u8 usbnet_hwaddr[6];
void copy_usb_lte_mac_addr(u8 *mac)
{
    puts("copy mac addr\n");
    memcpy(usbnet_hwaddr, mac, 6);
}


void get_usb_lte_mac_addr(u8 *mac)
{


    memcpy(mac, usbnet_hwaddr, 6);
}
 ...
 */
static void
lte_low_level_init(struct netif *netif)
{
    ...
    memcpy(netif->hwaddr, usbnet_hwaddr, 6);
 ...
 */
static err_t
lte_low_level_output(struct netif *netif, struct pbuf *p)
{
    struct pbuf *q;
    unsigned int i;
    u8 *ptxpkt;
    ...
    //获取txbd指向的内存地址
    extern u8 *rndis_get_tx_buf(void);
    ptxpkt = rndis_get_tx_buf();
    ...
    extern void rndis_tx_packet(int len);
    rndis_tx_packet(i);
 ...
 */
static struct pbuf *
lte_low_level_input(struct netif *netif)
{
    ...
    struct lte_module_pkg _lte_data = {0};
    s32 rndis_get_rxpkt_addr_len(struct lte_module_pkg * lte_module_pkg);
    ret = rndis_get_rxpkt_addr_len(&_lte_data);
    if (ret < 0) {
        return NULL;
    }
    /* printf("low_level_input len = %d \r\n",ret); */
    /* put_buf(_lte_data.data,ret); */
#if ETH_PAD_SIZE
    len += ETH_PAD_SIZE; /* allow room for Ethernet padding */
#endif

    /* We allocate a pbuf chain of pbufs from the pool. */
    p = pbuf_alloc(PBUF_RAW, _lte_data.data_len, PBUF_POOL);
...
//             dma0_cpy((void *)q->payload, (void *)&prxpkt[i], q->len);
            memcpy((void *)q->payload, (void *)&_lte_data.data[i], q->len);
            i += q->len;
        }

        //释放当前oeth_rxbd
        void rndis_rxpkt_addr_release(struct lte_module_pkg * lte_module_pkg);
        rndis_rxpkt_addr_release(&_lte_data);
#if ETH_PAD_SIZE
        pbuf_header(p, ETH_PAD_SIZE); /* reclaim the padding word */
#endif
 ...
 */
void
lte_ethernetif_input(struct netif *netif)
{
    struct pbuf *p;
    int err;
...
//  netif->state = ethernetif;
    ...
    netif->linkoutput = lte_low_level_output;

//  ethernetif->ethaddr = (struct eth_addr *)&(netif->hwaddr[0]);

    /* initialize the hardware */
    lte_low_level_init(netif);
    return ERR_OK;
}
