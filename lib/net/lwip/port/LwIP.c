...
int __attribute__((weak)) wifi_get_mac(u8 *mac)
{
    return 0;
}
...
void __lwip_renew(unsigned short parm)
{
    ...
    if (lwip_netif == WIFI_NETIF) {
        ...
        get_usb_lte_mac_addr(lte_netif.hwaddr);
        if (dhcp) {
...
//    printf("222  src 0x%x  dest 0x%x \n\n",src,dest);
    ...
    for (netif = netif_list; netif != NULL; netif = netif->next) {
        ...
        if (netif_is_up(netif) && netif_is_link_up(netif) && !ip4_addr_isany_val(*netif_ip4_addr(netif))) {
            ...
            /* printf("ip4_route v2 src %s \n\n", inet_ntoa(*src));                     */
            /* printf("ip4_route v2 dest %s \n\n", inet_ntoa(*dest));                           */
            /* printf("ip4_route v2 netif %s \n\n", inet_ntoa(*netif_ip4_addr(netif)));         */
