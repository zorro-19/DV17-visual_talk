...
void mbedtls_net_set_timeout(mbedtls_net_context *ctx, int to_ms)
{
    ctx->to_ms = to_ms;
}
int mbedtls_net_connect_bind(mbedtls_net_context *ctx, int domain, int socktype, int ai_protocol, char *ipaddr, u16 port)
{
    ...
    if (ctx->to_ms) {
        sock_set_recv_timeout((void *)ctx->hdl, ctx->to_ms);
        sock_set_send_timeout((void *)ctx->hdl, ctx->to_ms);
    }
...
int mbedtls_net_bind(mbedtls_net_context *ctx, const char *bind_ip, const char *port, int proto)
{
    ...
    for (cur = addr_list; cur != NULL; cur = cur->ai_next) {
        ...
        if (ctx->to_ms) {
            sock_set_recv_timeout(ctx->hdl, ctx->to_ms);
            sock_set_send_timeout(ctx->hdl, ctx->to_ms);
        }
                       ...
                       void *client_ip, size_t buf_size, size_t *ip_len)
{
    ...
    if (type != SOCK_STREAM) {
        ...
        if (bind_ctx ->to_ms) {
            sock_set_recv_timeout(bind_ctx->hdl, bind_ctx->to_ms);
            sock_set_send_timeout(bind_ctx->hdl, bind_ctx->to_ms);
        }
    }
