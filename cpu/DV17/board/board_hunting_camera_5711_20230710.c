...
UART3_PLATFORM_DATA_BEGIN(uart3_data)
	.baudrate =1000000,// 460800,//115200,
	/* .baudrate = 115200, */
...
LCD_PLATFORM_DATA_BEGIN(lcd_data)
    .lcd_name = "9225_mcu",
	.lcd_io = {
        .backlight = IO_PORTE_04,
        .backlight_value = 1,

		.lcd_reset   = -1,//IO_PORTE_05,//IO_PORTG_06,
		.lcd_standby = -1,
        .lcd_cs      = -1,
        .lcd_rs      = -1,
        .lcd_spi_ck  = -1,
...
static const struct camera_platform_data camera0_data = {
    .xclk_gpio      = IO_PORTA_15,
	.reset_gpio     = IO_PORTE_01,
	.pwdn_gpio      = IO_PORTE_00,
...
void turn_onoff_ir_cut(u8 state ){

    if(state==1){


           printf("\n  open ir cut..........\n  ");
            gpio_direction_output(IO_PORTG_07, 1); // 红外灯电源使能  1打开   0 关闭
            gpio_direction_output(IO_PORTB_11, 1); // led 指示灯灯    1 关闭  0 打开


           #if  1
            gpio_direction_output(IO_PORTH_06, 1); //开 ir_cut
            gpio_direction_output(IO_PORTH_07, 0);

          #else

            gpio_direction_output(IO_PORTH_06, 0); //关闭 ir_cut
            gpio_direction_output(IO_PORTH_07, 1);
          #endif

    }else if(state==0){

            printf("\n  closeir cut..........\n  ");
            gpio_direction_output(IO_PORTG_07, 0); // 红外灯电源使能  1打开   0 关闭
            gpio_direction_output(IO_PORTB_11, 1); // led 指示灯灯    1 关闭  0 打开
...
void set_io_measure_state(u8 on){


 gpio_direction_output(IO_PORTH_14, on);
...
ADKEY_PLATFORM_DATA_BEGIN(adkey_data)
	.io 		= IO_PORTB_14,//IO_PORTH_12,
	.ad_channel = AD_CH04_PB14,//AD_CH09_PH12,
	.table 	= {
		.ad_value = {
			ADC0_33,
			ADC0_30,
...
#define USB_WKUP_IO 	IO_PORTB_13
...
unsigned char usb_is_charging()
{
   ...
   // printf("\n gpio_read(USB_WKUP_IO):%d\n",gpio_read(USB_WKUP_IO));
...
static  u8  low_bat_flag=0;
unsigned int set_low_batery_state(u8 flag)
{
    low_bat_flag=flag;
}
unsigned int get_low_batery_state()
{
    return low_bat_flag;
}
...
POWER_PLATFORM_DATA_BEGIN(sys_power_data)
    .wkup_map = {
        ...
        {"wkup_pir", WKUP_IO_PR3, 0},
        //{0, 0, 0}
    },
    .voltage_table = {  //5.5  849 2.75    5.6  866 2.80      5.7 882  2.85     5.8 897  2.90      5.9 913  2.94       6.0  928  3.0     6.1 946  3.05         6.2 961 3.1          6.3  3.15  977     6.4  3.2  993
        {849-150, 10},
        {866-150, 20},   //775
        {928-150, 30},
        {932-150, 40},
    ...
    .min_bat_power_val = (918-150),
    .max_bat_power_val = (989-100),
...
WIFI_PLATFORM_DATA_BEGIN(wifi_data)
    .module = RTL8189F,
    .sdio_parm = SDIO_GRP_1 | SDIO_PORT_1 | SDIO_4_BIT_DATA/*|SDIO_DATA1_IRQ */|20 * 1000000,
    .wakeup_port = -1,//IO_PORTB_11,
    .cs_port = -1,
    .power_port = -1,//IO_PORTB_15,
WIFI_PLATFORM_DATA_END()
#endif
#endif
