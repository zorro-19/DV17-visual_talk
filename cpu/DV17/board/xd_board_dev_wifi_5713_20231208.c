...
LCD_PLATFORM_DATA_BEGIN(lcd_data)
    .lcd_name = "ST7785",
	.lcd_io = {
        .backlight = IO_PORTG_11,//IO_PORT_PR_02,
        .backlight_value = 1,

		.lcd_reset   = IO_PORTG_08,
		.lcd_standby = -1,
        .lcd_cs      = -1,
        .lcd_rs      = -1,
        .lcd_spi_ck  = -1,
...
static const struct camera_platform_data camera0_data = {
    .xclk_gpio      = IO_PORTE_02,// IO_PORTG_07,//
	.reset_gpio     = -1,
	.pwdn_gpio      = -1,
...
void switch_usb_workmode(u8 on){
   ...
   //gpio_direction_output(IO_PORTH_11, 1);//
    gpio_direction_output(IO_PORTH_09, 1);//connect PC

    }else{
    printf("\n CONNECT PC\n");
    gpio_direction_output(IO_PORTH_09, 0);//
   //gpio_direction_output(IO_PORTH_11, 0);//connect 7016
...
// 上电时电机矫正用
   ...
   // if(!net_config_mode){
    ...
    doorbell_irled_on();
    turn_360_degree(1,2,360,0 );//right
    extern void  stop_turn_degree();
    stop_turn_degree();
   // os_time_dly(50);

    turn_360_degree(3,2,60,0 );//up
    extern void  stop_turn_degree();
    stop_turn_degree();
   // os_time_dly(50);
    turn_360_degree(4,2,30,0 );//down
    extern void  stop_turn_degree();
    stop_turn_degree();

    turn_360_degree(2,2,180,0 );//left
    extern void  stop_turn_degree();
    stop_turn_degree();
    doorbell_irled_off();
...
// 1 为小机器usb 连接到电脑，  0 为小机和7016连接
void usb_switch_mode(u8 state){


    if(state){

   // gpio_direction_output(IO_PORTH_11, 1);//关闭DV17 和7016 usb
    gpio_direction_output(IO_PORTH_09, 1);//关闭DV17 和7016 usb

    }else{

    gpio_direction_output(IO_PORTH_09, 0);//使能DV17 和7016 usb
   // gpio_direction_output(IO_PORTH_11, 0);//使能DV17 和7016 usb
