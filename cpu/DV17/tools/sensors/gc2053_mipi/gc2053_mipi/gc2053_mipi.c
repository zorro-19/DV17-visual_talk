...
extern void *GC2053_MIPI_get_ae_params();
extern void *GC2053_mipi_get_awb_params();
extern void *GC2053_mipi_get_iq_params();

extern void GC2053_mipi_ae_ev_init(u32 fps);
...
unsigned char wrGC2053_MIPIReg(unsigned char regID, unsigned char regDat)
{
    u8 ret = 1;
...
unsigned char rdGC2053_MIPIReg(unsigned char regID, unsigned char *regDat)
{
    u8 ret = 1;
...
unsigned char wrGC2053_MIPIRegArray(u8 *array, u32 size)
{
...
void GC2053_MIPI_config_SENSOR(u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    u32 i;
    u8 val;

    for (i = 0; i < sizeof(GC2053_MIPI_INI_REG) / sizeof(Sensor_reg_ini); i++) {
        wrGC2053_MIPIReg(GC2053_MIPI_INI_REG[i].addr, GC2053_MIPI_INI_REG[i].value);
    }
...
s32 GC2053_MIPI_set_output_size(u16 *width, u16 *height, u8 *frame_freq)
{
    return 0;
}


s32 GC2053_MIPI_power_ctl(u8 isp_dev, u8 is_work)
{

    return 0;
}

s32 GC2053_MIPI_ID_check(void)
{
    u8 pid = 0x00;
    u8 ver = 0x00;
    u8 i ;
...
void GC2053_mipi_reset(u8 isp_dev)
{
    u32 reset_gpio;
    u32 pwdn_gpio;
...
s32 GC2053_MIPI_check(u8 isp_dev, u32 reset_gpio, u32 pwdn_gpio)
{
    puts("\n\n GC2053_MIPI_check \n\n");
    if (!iic) {
        if (isp_dev == ISP_DEV_0) {
...
s32 GC2053_MIPI_init(u8 isp_dev, u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    GC2053_mipi_reset(isp_dev);
    puts("\n\n GC2053_mipi_init22 \n\n");
...
s32 GC2053_mipi_get_mipi_clk(u32 *mipi_clk,  u32 *tval_hstt, u32 *tval_stto, u16 width, u16 height, u8 frame_freq)
{
    *mipi_clk = 297;
    *tval_hstt = 0;
    *tval_stto = 30;
...
u32 GC2053_mipi_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain)
{
    u32 texp;
    u32 texp_align;
    u32 ratio;
...
u32 GC2053_mipi_set_shutter(isp_ae_shutter_t *shutter)
{
    u32 gain;
    u32 again, dgain;
...
void GC2053_MIPI_sleep()
{
    wrGC2053_MIPIReg(0x3e, 0x90);
}

void GC2053_MIPI_wakeup()
{

    wrGC2053_MIPIReg(0x3e, 0x91);
}

void GC2053_MIPI_W_Reg(u16 addr, u16 val)
{
    //printf("update reg%x with %x\n", addr, val);
    wrGC2053_MIPIReg((u8)addr, (u8)val);
}

u16 GC2053_MIPI_R_Reg(u16 addr)
{
    u8 val;
...
REGISTER_CAMERA(GC2053_MIPI) = {
    ...
    .ops                =   {
        ...
        .get_ae_params  	=	GC2053_MIPI_get_ae_params,
        .get_awb_params 	=	GC2053_mipi_get_awb_params,
        .get_iq_params 	    =	GC2053_mipi_get_iq_params,
