...
#include "spec_gc2053_mipi_ae.h"
#include "spec_gc2053_mipi.h"

#include "asm/isp_alg.h"


extern u32 spec_GC2053_mipi_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 spec_GC2053_mipi_set_shutter(isp_ae_shutter_t *shutter);
extern void  *spec_GC2053_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz

const static u32 GC2053_mipi_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 GC2053_mipi_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
const static u32 GC2053_mipi_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  //lv = 20
...
static isp_ae_params_t  GC2053_mipi_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = spec_GC2053_mipi_calc_shutter,
        .set_shutter  = spec_GC2053_mipi_set_shutter,
        .get_ae_curve = spec_GC2053_mipi_get_ae_curve,
    },

#if 1
    .ae_hl_comp_en = 1,
    .ae_hl_count_th0 = 5000,
    .ae_hl_count_th1 = 8000, // 10000
    .ae_hl_lv_th = 1024 * 2, // 8192
    .ae_hl_comp_max = 1024 * 18 / 10, // x2 gain
#endif // 0

};


void spec_GC2053_mipi_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = spec_GC2053_mipi_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    spec_GC2053_mipi_calc_shutter(&ae_shutter, time, gain);

    spec_GC2053_mipi_set_shutter(&ae_shutter);
}


void   *spec_GC2053_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    const void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = GC2053_mipi_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = GC2053_mipi_ae_50hz_30fps;
        }
    } else {
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;

        curve = GC2053_mipi_ae_60hz_30fps;
...
void *spec_GC2053_MIPI_get_ae_params()
{
    return (void *)&GC2053_mipi_ae_params;
}
