...
#define PR2000K_AUTO_RESO_ENABLE   0
...
static u8 cur_reso = PR2000K_HD1080p;

#define WRCMD 0xB8
#define RDCMD 0xB9

static void pr2000k_720p_init();
static void pr2000k_1080p_init();

static void pr2000k_set_isp_size(u32 width, u32 height, u32 fps)
{
    isp_sen_t *c;
    list_for_each_camera(c) {
        if (!strcmp((const char *)(c->logo), "PR2000K")) {
            c->sen_size.w =  	width;
            c->sen_size.h =  	height;
            c->isp_size.w = 	width;
            c->isp_size.h = 	height;
...
static int pr2000k_format_det()
{
#if (PR2000K_AUTO_RESO_ENABLE)
    u8 DetVideo = 0;
    u8 det_cnt = 20;
    u8 reso = 0;
    ...
    /* } */
    wrPR2000KReg(0xff, 0x00);
    for (u8 j = 0; j < det_cnt; j++) {
        rdPR2000KReg(0x00, &DetVideo);
        log_d("\n%s %d DetVideo:%x\n", __func__, __LINE__, DetVideo);
        if (DetVideo != 0) {
            reso = DetVideo & (0x03);
            log_d("\n%s %d Has Video:%x  cur_reso:%x\n", __func__, __LINE__, DetVideo, reso);
            return reso;
        }
        delay(1000000);
    }
...
static s32 pr2000k_initialize(u8 isp_dev, u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    ...
    pr2000k_1080p_init();
    cur_reso = PR2000K_HD1080p;
    pr2000k_set_isp_size(1920, 1080, 25);


#if (PR2000K_AUTO_RESO_ENABLE)
    reso = pr2000k_format_det();
    if (cur_reso != reso) {
        switch (reso) {
        case PR2000K_HD720p:
            pr2000k_720p_init();
            pr2000k_set_isp_size(1280, 720, 25);
            break;
        case PR2000K_HD1080p:
            pr2000k_1080p_init();
            pr2000k_set_isp_size(1920, 1080, 25);
            break;
        default:
