...
#include "sc2313_mipi_ae.h"
#include "sc2313_mipi.h"

#include "asm/isp_alg.h"


extern u32 sc2313_mipi_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 sc2313_mipi_set_shutter(isp_ae_shutter_t *shutter);
extern void  *sc2313_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz
const static u32 sc2313_mipi_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {

    // EV       ExpoTime    ISO     INTERPOLATE
    {21004615,     13,       100,    AE_INTERP_EXP},  // lv = 20
    ...
    {26,        40000,      12800 * 2,    AE_INTERP_GAIN},
    {13,        80000,      12800 * 2,    AE_INTERP_GAIN},
    //{26,        60000,      8533*2,    AE_INTERP_GAIN},
    {AE_CURVE_END, 0,         0,              0}
};

const static u32 sc2313_mipi_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {21004615,     13,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    10000,       200,    AE_INTERP_GAIN},
    ...
    {26,        30000,      8533 * 4,    AE_INTERP_GAIN},
    //{26,        60000,      8533*2,    AE_INTERP_GAIN},
    {AE_CURVE_END, 0,         0,              0}
};

const static u32 sc2313_mipi_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {

    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000 * 2,     15,       100,    AE_INTERP_EXP}, //lv = 20
...
static isp_ae_params_t  sc2313_mipi_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = sc2313_mipi_calc_shutter,
        .set_shutter = sc2313_mipi_set_shutter,
        .get_ae_curve = sc2313_mipi_get_ae_curve,
    },
};


void sc2313_mipi_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = sc2313_mipi_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    sc2313_mipi_calc_shutter(&ae_shutter, time, gain);

    sc2313_mipi_set_shutter(&ae_shutter);
}

void   *sc2313_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    const void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = sc2313_mipi_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = sc2313_mipi_ae_50hz_30fps;
        }
    } else {
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;

        curve = sc2313_mipi_ae_60hz_30fps;
...
void *sc2313_mipi_get_ae_params()
{
    return (void *)&sc2313_mipi_ae_params;
}
