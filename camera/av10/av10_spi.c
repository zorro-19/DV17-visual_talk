...
/******************************************************************************************/
static int spi_init(const struct dev_node *node, void *_data)
{
...
static int  spi_open(const char *name,  struct device **device, void *arg)
{
    struct avin_spi_device *p;
...
static int spi_read(struct device *device, void *buf, u32 len, u32 addr)
{
...
static int spi_write(struct device *device, void *buf, u32 len, u32 addr)
{
    struct avin_spi_device *spi = (struct avin_spi_device *)device->private_data;
    if (spi->ops->write) {
        os_mutex_pend(&spi->mutex, 0);
...
static int spi_ioctl(struct device *device, u32 cmd, u32 arg)
{
    struct avin_spi_device *spi = (struct avin_spi_device *)device->private_data;
    if (spi->ops->ioctl) {
        os_mutex_pend(&spi->mutex, 0);
...
//  SPI send
//------------------------------
static int _spi_send(const void *_spi, void *buf, int len)
{
    u8 i;
    const struct software_spi *spi = (const struct software_spi *)_spi;
...
//  SPI receive
//------------------------------
static int _spi_receive(const void *_spi, void *buf, int len)
{
    const struct software_spi *spi = (const struct software_spi *)_spi;
...
static int _spi_ioctrl(const void *_spi, int cmd, int arg)
{
    const struct software_spi *spi = (const struct software_spi *)_spi;
...
//      av10 spi1
//-----------------------------
static int _spi_ini(const void *_spi)
{
    const struct software_spi *spi = (const struct software_spi *)_spi;
    //printf("\nAV10 spi io initial \n");
    ISP_CS_H(spi);
