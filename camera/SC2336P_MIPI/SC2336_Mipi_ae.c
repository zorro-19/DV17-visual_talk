...
#include "SC2336_mipi_ae.h"
#include "SC2336_mipi.h"

#include "asm/isp_alg.h"


extern u32 SC2336_mipi_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 SC2336_mipi_set_shutter(isp_ae_shutter_t *shutter);
extern void  *SC2336_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz
const static u32 SC2336_mipi_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
  ...
  //  {4,       500000,     12800,    AE_INTERP_GAIN},
    {AE_CURVE_END, 0,         0,              0}
};

const static u32 SC2336_mipi_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
const static u32 SC2336_mipi_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
...
static isp_ae_params_t  SC2336_mipi_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = SC2336_mipi_calc_shutter,
        .set_shutter = SC2336_mipi_set_shutter,
        .get_ae_curve = SC2336_mipi_get_ae_curve,
    },
    ...
    .ae_hl_lv_th = 1024*2,
    .ae_hl_comp_max = 1024*15/10,

};


void SC2336_mipi_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = SC2336_mipi_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    SC2336_mipi_calc_shutter(&ae_shutter, time, gain);

    SC2336_mipi_set_shutter(&ae_shutter);
}

void   *SC2336_mipi_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    const void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = SC2336_mipi_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = SC2336_mipi_ae_50hz_30fps;
        }
    } else {
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;

        curve = SC2336_mipi_ae_60hz_30fps;
...
void *SC2336_mipi_get_ae_params()
{
    return (void *)&SC2336_mipi_ae_params;
}
